...
template HashElemsToField(numElems) {
    ...
    // TODO - We need to update the forked circom we use to build to for 2.1.15 to go back to supporting
    //        these being declared in the if statements (to prevent them from being allocated for small numElements)
    signal inputs_one[16];
    signal inputs_two[16];
    signal inputs_three[16];
    signal inputs_four[16];
    ...
    if (numElems <= 16) { 
        ...
        h1 <== Poseidon(16)(inputs_one);
        h2 <== Poseidon(numElems-16)(inputs_two);
        ...
        h1 <== Poseidon(16)(inputs_one);
        h2 <== Poseidon(16)(inputs_two);
        h3 <== Poseidon(numElems-32)(inputs_three);
        ...
        h1 <== Poseidon(16)(inputs_one);
        h2 <== Poseidon(16)(inputs_two);
        h3 <== Poseidon(16)(inputs_three);
        h4 <== Poseidon(numElems-48)(inputs_four);
